                        ...
                        <table class="table table-bordered" id="tb1" width="100%" cellspacing="0">
                            ...
                            <tbody>
                                <?php
                                    if(!$eventIdPost)
                                    {
                                        $query = "SELECT * FROM `participants` WHERE `ticket_sent` = '0' AND `partner` = '$user_partner'";
                                        $sql = mysqli_query($con,$query);
                                        while($row = mysqli_fetch_assoc($sql))
                                        {
                                    ...
                                    {
                                        $query = "SELECT * FROM `participants` WHERE `ticket_sent` = '0' AND `event_id` = '$eventIdPost' AND `partner` = '$user_partner'";
                                        $sql = mysqli_query($con,$query);
                                        while($row = mysqli_fetch_assoc($sql))
                                        {
                                            $subscriberName = $row['name'];
